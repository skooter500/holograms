...
@onready var desired_engine_pitch: float = 1
...
func _physics_process(delta: float):
	...
	if DisplayServer.is_touchscreen_available() or Input.is_action_pressed(&"accelerate") or accel > 0:
...
func _on_left_xr_input_float_changed(name: String, value: float) -> void:
